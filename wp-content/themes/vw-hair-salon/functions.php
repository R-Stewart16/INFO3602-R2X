...
function vw_hair_salon_scripts() {
    ...
    wp_enqueue_script('main-search-js', get_theme_file_uri('/js/scripts-bundled.js'), array('jquery'), '1.0',true);
	wp_localize_script('main-search-js',
		'universityData',
		array(
...
/** /* Redirect Conrtibutor accounts out of admin and onto home page */
...
function redirectToFrontend(){
	...
	if($userNumRoles == 1 AND $userRole == 'contributor' OR $userRole == 'author'){
		wp_redirect(site_url('/'));
		exit;
	}
}

/* Hide admin bar from Contributors */
add_action('wp_loaded', 'noContriAdminBar');

function noContriAdminBar(){
	$ourCurrentUser = wp_get_current_user();
	$userNumRoles = count($ourCurrentUser->roles);
	$userRole = $ourCurrentUser->roles[0];
	if($userNumRoles == 1 AND $userRole == 'contributor' OR $userRole == 'author'){
